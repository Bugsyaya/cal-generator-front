...
<!--    <div>
    ...
    <table>
      <tr v-for="item in tableData2" :key="item.ind">
        <th> {{item.ind+1 }}</th>
        <th> {{ item.lib }} </th>
        <th><select multiple class="selectpicker">
          <option v-for="se in item.modulePossible" :key="se.id"> {{ se.id }}</option>
        </select></th>
      </tr>
    </table>
...
export default {
  ...
  methods: {
    async getModules (callback) {
      /* var obj = {} */
      await api.getModulesByCodeFormation(this.$route.params.id)
        .then(response => {
    ...
    ajoutModule: function (posDansListe, event) {
      ...
      if (tailleTableData === 0) {
        ...
        for (let i = 0; i < tailleSelection; i++) {
          // console.log("loop i: ", i)
          obj['id'] = listeSelection[i].label
          for (let j = 0; j < tailleTableData; j++) {
      ...
      if (tailleSelection > 0) {
        for (let i = 0; i < tailleTableData; i++) {
          // console.log("loop i: ", i)
          isEgale = 0
          for (let j = 0; j < tailleSelection; j++) {
            // console.log("loop j: ", j)
    ...
    addFromPossible: function (posDansListe) {
      var isPresent = 0
      var obj = {}
      for (let i = 0; this.tableData.length; i++) {
        for (let j = 0; j < this.tableData[i].modulePossible.length; j++) {
          if (isPresent === 0) {
    ...
    removeFromPossible: function (placeDuModuleASupp, idListeAmodif) {
      ...
      for (let i = 0; i < tailleListePossible; i++) {
